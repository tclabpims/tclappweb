    ...
    <div class="pagemain">
        ...
        <table class="sui-table table-bordered" style="margin-top:20px;">
            ...
            <tr>
                ...
                <th class="center left-con" width="32%">套餐名称</th>
                <th class="center left-con" width="30%">销售数量（个）</th>
                <th class="center left-con" width="30%">销售额（元）</th>
            </tr>
            </thead>
            <tbody id="all_task0">
            ...
            <c:forEach items="${list}" var="item">
                <tr>
                    <td class="center left-con">${item.packageId}</td>
                    <td class="center left-con">${item.packageName}</td>
                    <td class="center left-con">${item.saleTotalNumber}</td>
                    <td class="center left-con">${item.saleSum}</td>
                </tr>
            </c:forEach>
            </tbody>
    </table>
...
<div align="center" class="page_num_style">
    ...
    <c:choose>
        ...
        <c:otherwise>
            <c:choose>
                <c:when test="${pageNo > 1}">
                    <a href="${pageContext.request.contextPath}/finaManage/packageSaleCondition.do?pageNo=${pageNo - 1}">上一页</a>&nbsp;&nbsp;
                </c:when>
                <c:otherwise>
                    <a href="#">上一页</a>&nbsp;&nbsp;
                </c:otherwise>
            ...
            <c:choose>
                <c:when test="${pageNo < totalPage}">
                    <a href="${pageContext.request.contextPath}/finaManage/packageSaleCondition.do?pageNo=${pageNo + 1}">下一页</a>&nbsp;&nbsp;
                </c:when>
                <c:otherwise>
                    <a href="#">下一页</a>&nbsp;&nbsp;
                </c:otherwise>
