...
public class ExcelExportUtil {
    ...
    public static void orderRecoredExport(HttpServletResponse response, List<OrderModelWithBLOBs> orderList) {
        //标题行
        String title[] = {"我方条码号", "病人姓名", "病人唯一号", "门诊住院号", "性别（1男2女）", "年龄",
...
//			response.setHeader("Content-Disposition", "attachment;filename=" + new String((filename).getBytes(), "iso-8859-1"));
            ...
            for (int i=0; i<orderList.size(); i++) {
                ...
                if (orderList.get(i).getApplicantModel() != null && orderList.get(i).getApplicantModel().getSex() != null && orderList.get(i).getApplicantModel().getSex().equals("男")) {
                    sheet.addCell(new Label(j++, i+1, "1", format));
                } else if (orderList.get(i).getApplicantModel() != null && orderList.get(i).getApplicantModel().getSex() != null && orderList.get(i).getApplicantModel().getSex().equals("女")) {
                    sheet.addCell(new Label(j++, i+1, "2", format));
                } else {
                ...
                sheet.addCell(new Label(j++, i + 1, StringUtil.priceToString(orderList.get(i).getPrice()), format));
            }
            //写入数据
            book.write();
            //关闭文件
