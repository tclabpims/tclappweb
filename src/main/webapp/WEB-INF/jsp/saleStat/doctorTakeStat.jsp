...
<div class="nr">
    ...
    <div class="pagemain">
        ...
        <input id="strSubMenuId" type="hidden" value="84"/>
        <%--查询 start--%>
        <div>
            <br/>
            <form action="${pageContext.request.contextPath}/doctorTakeStat/query.do" METHOD="post">

                &nbsp;&nbsp;&nbsp;<label class="label_Style">采集医生姓名</label>
                &nbsp;&nbsp;&nbsp;<input type="text" class="input_text_style" name="takeDoctorName" value="${takeDoctorName}" />
        ...
        <table class="sui-table table-bordered" style="margin-top:20px;">
            ...
            <tr>
                ...
                <th class="center left-con" width="20%">采集医生姓名</th>
                <th class="center left-con" width="20%">采集量（个）</th>
                <th class="center left-con" width="20%">采集总额（元）</th>
...
<div id="table_stat" style="display: none">
    ...
    <table class="sui-table table-bordered" style="margin-top:20px;width: 700px">
        ...
        <tr>
            <th class="center left-con" width="8%">编号</th>
            <th class="center left-con" width="17%">采集医生编号</th>
            <th class="center left-con" width="20%">采集医生姓名</th>
            <th class="center left-con" width="20%">采集量（个）</th>
            <th class="center left-con" width="20%">采集总额（元）</th>
        ...
        <c:set var="totalTakeAmount" value="${0}" />
        <c:forEach items="${list_all}" var="item">
            <tr>
                ...
                <td class="center left-con">${item.takeNum}</td>
                <td class="center left-con"><fmt:formatNumber type="number" value="${item.takeAmount / 100}" pattern="#0.00" maxFractionDigits="2"/> </td>
                <c:set var="totalTakeAmount" value="${item.takeAmount + totalTakeAmount}" />
    ...
    <br>
    <p align="right">总计：<fmt:formatNumber type="number" value="${totalTakeAmount / 100}" pattern="#0.00" maxFractionDigits="2" />（元） </p>
</div>
...
<div align="center" class="page_num_style">
    ...
    <c:choose>
        <c:when test="${query_flag == true}">
            <c:choose>
                <c:when test="${pageNo > 1}">
                    <a href="${pageContext.request.contextPath}/doctorTakeStat/query.do?pageNo=${pageNo - 1}&takeDoctorName=${takeDoctorName}&year=${year}&month=${month}">上一页</a>&nbsp;&nbsp;
                </c:when>
                <c:otherwise>
                    <a href="#">上一页</a>&nbsp;&nbsp;
                </c:otherwise>
            ...
            <c:choose>
                <c:when test="${pageNo < totalPage}">
                    <a href="${pageContext.request.contextPath}/doctorTakeStat/query.do?pageNo=${pageNo + 1}&takeDoctorName=${takeDoctorName}&year=${year}&month=${month}">下一页</a>&nbsp;&nbsp;
                </c:when>
                <c:otherwise>
                    <a href="#">下一页</a>&nbsp;&nbsp;
                </c:otherwise>
        ...
        <c:otherwise>
            <c:choose>
                <c:when test="${pageNo > 1}">
                    <a href="${pageContext.request.contextPath}/doctorTakeStat/list.do?pageNo=${pageNo - 1}">上一页</a>&nbsp;&nbsp;
                </c:when>
                <c:otherwise>
                    <a href="#">上一页</a>&nbsp;&nbsp;
                </c:otherwise>
            ...
            <c:choose>
                <c:when test="${pageNo < totalPage}">
                    <a href="${pageContext.request.contextPath}/doctorTakeStat/list.do?pageNo=${pageNo + 1}">下一页</a>&nbsp;&nbsp;
                </c:when>
                <c:otherwise>
                    <a href="#">下一页</a>&nbsp;&nbsp;
                </c:otherwise>
...
<%--分页 end--%>
<script type="text/javascript" src="${pageContext.request.contextPath}/js/saleStat/doctorTakeStat.js"></script>
</body>
</html>
